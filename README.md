...
git clone https://github.comcd logistics-ai-orchestrator

Configure your localized environment file: Create an environment file named LLM_API.env inside your designated configuration folder containing your Google GenAI credentials:
GOOGLE_API_KEY="your_free_google_gemini_api_key_here"

Install the required technical library bundle:
pip install -r requirements.txt

Launch the interactive Terminal Agent Loop:
python agent_cli.py
...
This architecture is currently a fully functional production prototype optimized for single-node local execution. The following structural boundaries have been intentionally mapped to be resolved in the next development phase:
🛡️ Enterprise Query Guardrails (Security Phase)
...
📥 In-Memory Data Export Arrays (Delivery Phase)
...
🌐 Web Interface Migration (Presentation Phase)
...
⚡ API Quota & Scale Hardening (Orchestration Phase)
...
🔄 Continuous Metadata Sync & Schema Evolution (Data Pipeline Phase)
